...
public abstract class InputParserBase<TInput> {
     ...
     */
    final List<CalculateChangeArgs> parse(final TInput input) {
        final List<CalculateChangeArgs> calculateChangeArgsList = this.parseImpl(input);
     ...
     */
    protected abstract List<CalculateChangeArgs> parseImpl(final TInput input);
}
