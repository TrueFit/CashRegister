...
class PhysicalSetConfigParserResultCollection extends AbstractCollectionDecorator<PhysicalSetConfigParserResult> {
     ...
     */
    PhysicalSetConfigParserResultCollection(final Collection<PhysicalSetConfigParserResult> collection) {
        super();
    ...
    private Collection<Throwable> collectValidationExceptions() {
        this.doValidate();
        final List<Throwable> primaryValidationExceptions = this.decorated
                .parallelStream()
                .map(PhysicalSetConfigParserResult::getJsonParseException)
                .filter(Objects::nonNull)
                .collect(Collectors.toUnmodifiableList());
        final List<Throwable> secondaryValidationExceptions = this.decorated
                .parallelStream()
