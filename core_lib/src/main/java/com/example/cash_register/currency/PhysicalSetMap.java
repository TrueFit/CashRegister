 ...
 * parse all JSON files in the resource directory {@code `/com/example/cash_register/currency/physical_sets/`}.  The
 * results of this parsing are placed into this object and made available as a Spring component.
 */
...
public class PhysicalSetMap extends ForwardingSortedSetMultimap<Currency, PhysicalCurrency> {
     ...
     * @param physicalSetConfigParser Physical set config parser.
     */
    @Autowired
    private PhysicalSetMap(
            final CurrencyModelMapper currencyModelMapper,
            final PhysicalSetConfigParser physicalSetConfigParser) {
        String locationPattern = "/com/example/cash_register/currency/physical_sets/**/*.json";
        PhysicalSetConfigParserResultCollection parserResultCollection = physicalSetConfigParser.parseResources(locationPattern);
        SortedSetMultimap<Currency, PhysicalCurrency> mappedParserResultCollection = currencyModelMapper.mapToSortedSetMultimap(parserResultCollection);
