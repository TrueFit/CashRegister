...
public abstract class ChangeCalculatorBase {
    /**
     * Change args validator.
     *
     * @see CalculateChangeArgs.Validator
     */
    @Autowired
    ...
    final CalculateChangeResult calculate(final CalculateChangeArgs args) {
        ...
        if (args.isValid()) {
            ...
            final InvalidCalculateChangeArgsException exception = new InvalidCalculateChangeArgsException(
                    cause);
            Collection<Throwable> causeList = ExceptionUtils.getThrowableList(exception);
            results = CalculateChangeResult.unsuccessful(args, causeList);
        }
        return results;
    }
