...
* a JDK, version 13.0.1
* an internet connection and access to the Maven Central artifact repository
* a graphical environment
  * the graphical environment is needed for the `tango_demo_app` module.  The other modules in this project
  do not need a graphical environment.
  * the (graphical) tests of the `tango_demo_app` module are disabled by default, since only local
  Linux execution of these tests is currently supported.  These tests can be enabled by setting the Gradle
  project property `tango_demo_app_tests` (a property value is not needed).  For example, via the command line
  argument `-Ptango_demo_app_tests`.
  * For reference: if a Linux machine is unavailable, a video demonstrating an execution of
  `./gradlew clean build -Ptango_demo_app_tests`, which shows the graphical tests running is available. The
  `tango_demo_app` tests can be seen at approximately 42 seconds into the video.
    * ![artifacts/clean_build_with_tango_demo_app_tests_example.mp4](artifacts/clean_build_with_tango_demo_app_tests_example.mp4 "./gradlew clean build -Ptango_demo_app_tests")
...
[java.util.Currency](https://docs.oracle.com/en/java/javase/13/docs/api/java.base/java/util/Currency.html) class,
...
* **`tango_lib`**: Tango modules primarily support the requirements as specified in the original README.  This library
contains the classes which support tango properties, input parsing, change calculation, and result output.  These 
classes are used to parse the expected sample input and to perform both the "normal" (fewest number of pieces)
and "random" change calculations, per the original README.
...
* Linux and macOS:  `./gradlew clean build`
  * to enable the tests for the `tango_demo_app` module: `./gradlew clean build -Ptango_demo_app_tests`
...
ensure support for edge cases during change calculations. The supported currencies are:
...
[Locale.Builder.setLanguageTag(String)](https://docs.oracle.com/en/java/javase/13/docs/api/java.base/java/util/Locale.Builder.html#setLanguageTag(java.lang.String))
...
![](artifacts/tango_demo_app_select_file.png "Tango Demo App, Select File")

###### Tango sample file output

![](artifacts/tango_demo_app_results_sample_file.png "Tango Demo App, Sample File Output")

###### Tango file output with errors

![](artifacts/tango_demo_app_results_tango_file_with_errors.png "Tango Demo App, Tango File Output With Errors")

###### JSON file output

![](artifacts/tango_demo_app_results_json_file.png "Tango Demo App, JSON File Output")
...
![](artifacts/repl_demo_app_page_1.png "Page 1")

![](artifacts/repl_demo_app_page_2.png "Page 2")
...
* An output list of calculate change results, which, in turn, are made up of:
  ...
  * The change calculator class used to generate the result
  * The output change value, or `null`, if no change is available 
  * The output bag of physical currency representing the change, or empty, if no change is available.
  * The list of exceptions associated with this result, or empty, if no exceptions are present. Generally, these
  will be validation exceptions describing issues with the change calculation operation.
...
```json
{
  ...
  "outputList" : [ {
    "uuid" : "b828e7e3-bb33-4b9e-a199-350236e542a2",
    "inputArgs" : {
      "owed" : "$3.00 (USD)",
      "tendered" : "$5.50 (USD)"
    },
    "changeCalculatorClass" : "com.example.cash_register.currency.ChangeCalculatorFewest",
    "outputChangeValue" : "$2.50 (USD)",
    "outputChangeBag" : "[2:dollar bill [$1.00 (USD)],2:quarter [$0.25 (USD)]]",
    "outputResultExceptionList" : [ ]
  }, {
    "uuid" : "0cb3938c-c744-44b3-8e5b-1faeece8ee29",
    "inputArgs" : {
      "owed" : "$3.50 (USD)",
      "tendered" : "$1.00 (USD)",
    ...
    "uuid" : "8e7dfcae-3f09-4756-ac86-93c8fa7d7cbd",
    "inputArgs" : {
      "owed" : "$2.50 (USD)",
      "tendered" : "$5.00 (USD)"
    },
...
* ![artifacts/javadoc.tar.xz](artifacts/javadoc.tar.xz "JavaDoc output")

The documents covers the main Java files, and some of the utility and helper methods used by test files.  Test files, 
where not documented via Javadoc, tend to have the test classes named after the classes being tested. Within the test
classes themselves, the `@Test` methods tend to be named after the method they are testing and (if needed) the
exception they are expecting to be thrown.
...
* ![artifacts/jacoco_reports.tar.xz](artifacts/jacoco_reports.tar.xz "JaCoCo output")
